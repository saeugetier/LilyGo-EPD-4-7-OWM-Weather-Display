...
function doGet(e) {
  ...
  for (var ii = 0; ii < calendars.length; ii++) {
    if (calendars[ii].isSelected() && calendars[ii].getName() != "Kalenderwochen") {
      calendars_selected.push(calendars[ii]);
      Logger.log(calendars[ii].getName());
    }
    else
    {
      Logger.log("Ausgeblendet: " + calendars[ii].getName());
    }
  }
  ...
  for (var ii = 0; ii < events.length; ii++) {
    ...
    str += event.getStartTime().toLocaleString() + ';' +
    event.getEndTime().toLocaleString() + ';' +
    event.getTitle() +';' + 
    event.isAllDayEvent() + '\r\n';
  }
